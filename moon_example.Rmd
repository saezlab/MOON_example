...
while (before != after & i < 10) {
  ...
  meta_network_compressed_current <- filter_incohrent_TF_target(recursive_decoupleRnival_res, TFN, meta_network_compressed_current, RNA_input)
...
recursive_decoupleRnival_res <- recursive_decoupleRnival_res[,c(4,2,3)]
...
recursive_decoupleRnival_res <- merge(recursive_decoupleRnival_res, RNA_data, by = "source_original",all.x = T)
...
names(SIF)[3] <- "sign"
write_csv(recursive_decoupleRnival_res, file = "results/node_attributes.csv")
